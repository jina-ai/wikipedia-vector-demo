...
const DDCCategories: [string, string][] = [
    ['000', 'Generalities'],
    ['001', 'Knowledge'],
    ['002', 'The book'],
    ['003', 'Systems'],
    ...
    ['891', 'Iranian/Persian Literature'],
    ['892', 'Celtic/Gaelic Literature'],
    ['893', 'Slavic and Baltic Literatures'],
...
].filter(([, desc]) => !desc.startsWith('Not assigned')) as [string, string][];

@singleton()
export class DDCEmbedder extends AsyncService {
    ...
    async indexCategoryBatch(...batch: [string, string][]) {
        const descs = batch.map(([code, desc]) => `${desc}`);
        const r = await this.embeddingsAPI.embedText(descs, 'jina-embeddings-v5-text-small', 'classification');
        ...
        const docs = batch.map(([code, desc], i) => {
            return {
                name: desc,
                identifier: code,
                abstract: `${code}: ${desc}`,
                vector: this._tensorToElasticBase64(tensors[i]),
            };
        });

        const ops = docs.flatMap((doc) => {
            return [{ index: { _index: this.esIndexName, _id: doc.identifier } }, doc];
        });

        const bulkIndex = await this.esClient.bulk({
            refresh: true,
            operations: ops,
        });
        if (bulkIndex.errors) {
            throw new Error('Failed to index DDC categories batch');
        }
    ...
    async indexAllCategories() {
        const batchSize = 20;
        for (let i = 0; i < DDCCategories.length; i += batchSize) {
            const batch = DDCCategories.slice(i, i + batchSize);
            await this.indexCategoryBatch(...batch);
        }
    }
